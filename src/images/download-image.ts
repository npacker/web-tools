...
import { followRedirects, isRetryableFetchError, readLimitedBytes } from "../http"
...
async function attemptImageFetch(url: string, impit: Impit, signal: AbortSignal): Promise<ImpitResponse> {
  ...
  return followRedirects(impit, url, {
    signal,
    fetchSignal: combinedSignal,
    timeoutMessage: `Request timed out after ${IMAGE_DOWNLOAD_TIMEOUT_MS}ms`,
  })
}
