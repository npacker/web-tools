...
7. **Return** — Web search returns `{ title, url, snippet?, date?, type?, description? }` records (snippet omitted when `includeSnippets` is disabled; the three metadata keys are omitted when extraction yielded nothing for that result). Image Search returns `{ image, title?, sourcePage? }` records — `image` is the local path of a downloaded file, or the remote URL when the download failed; `title` and `sourcePage` come from Bing's tile metadata and are omitted when absent. Image Search and View Images download files via [src/images/download-images.ts](src/images/download-images.ts) into the per-chat working directory obtained from `ctl.getWorkingDirectory()`. That call is made inside each tool's `implementation` (not at `toolsProvider` setup), since the SDK only attaches a working directory when a tool is actually invoked from a chat. Visit Website ([src/website/fetch-website.ts](src/website/fetch-website.ts)) returns only the page title, first-level headings, and a readable-content excerpt — no image download or link extraction. View Images accepts explicit URLs and/or scrapes images from a given page, returning per-image records with filename, alt, title, and a markdown reference to the downloaded file.
...
Cache sizes and subdirs are defined in [src/tools-provider.ts](src/tools-provider.ts); the `TTLCache` implementation is in [src/cache/ttl-cache.ts](src/cache/ttl-cache.ts). TTL defaults live in [src/config/resolve-config.ts](src/config/resolve-config.ts). Image search is **not** cached: the JSON metadata Bing returns is small, the cost of a bypassed fetch is bounded by the shared rate limiter, and the per-image downloads themselves bypass cache because they land in chat-scoped working directories.
