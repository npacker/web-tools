...
Cache sizes and subdirs are defined in [src/tools-provider.ts](src/tools-provider.ts); the `TTLCache` implementation is in [src/cache/ttl-cache.ts](src/cache/ttl-cache.ts). TTL defaults live in [src/config/resolve-config.ts](src/config/resolve-config.ts). Image search is **not** cached: the JSON metadata Bing returns is small and the cost of a bypassed fetch is bounded by the shared rate limiter. Fetch Images downloads are also uncached because they land in chat-scoped working directories.
